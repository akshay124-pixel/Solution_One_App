...
const ProductionApproval = () => {
  ...
  useEffect(() => {
    const socket = io(`${socketOrigin}/furni`, {
  ...
  const fetchOrders = useCallback(async () => {
    try {
      const response = await furniApi.get("/api/production-approval-orders");
      setOrders(response.data.data);
    } catch (error) {
      console.error("Error fetching production approval orders:", error);
      let errorMessage = "Something went wrong while loading orders. Please try again.";
      if (error.response) {
    ...
    if (searchLower) {
      filtered = filtered.filter((order) => {
        ...
        const productFields = (order.products || []).map((p) =>
          [p.productType, p.size, p.spec, p.gst, p.serialNos?.join(", "), p.modelNos?.join(", "), String(p.qty), String(p.unitPrice)]
            .filter(Boolean).map((field) => String(field).toLowerCase()).join(" ")
        );
        ...
        if (matchingProducts.length > 0) {
          totalProductQuantity += matchingProducts.reduce((sum, p) => sum + (p.qty || 0), 0);
        }
        return matchesFields || matchingProducts.length > 0;
      });
    } else {
      totalProductQuantity = filtered.reduce(
        (sum, order) => sum + (order.products || []).reduce((qty, p) => qty + (p.qty || 0), 0),
        0
      );
    }
    ...
    <>
      <div style={{ background: "linear-gradient(135deg, #2575fc, #6a11cb)", padding: "25px 40px", display: "flex", flexWrap: "wrap", gap: "20px", alignItems: "center", justifyContent: "space-between" }}>
        <Form.Control
          ...
          style={{ maxWidth: "450px", padding: "14px 25px", borderRadius: "30px", border: "none", background: "rgba(255,255,255,0.95)", boxShadow: "0 5px 15px rgba(0,0,0,0.2)", fontSize: "1.1rem", fontWeight: "500" }}
        />
        <Button onClick={handleExportToXLSX} style={{ background: "linear-gradient(135deg, #28a745, #4cd964)", border: "none", padding: "10px 20px", borderRadius: "5px", color: "#fff", fontWeight: "600", textTransform: "uppercase" }}>
          Export to XLSX
        </Button>
      </div>
      <div style={{ minHeight: "100vh", background: "linear-gradient(135deg, #e6f0fa, #f3e8ff)", padding: "30px", fontFamily: "'Poppins', sans-serif" }}>
        ...
        <div style={{ background: "white", borderRadius: "12px", boxShadow: "0 10px 30px rgba(0,0,0,0.2)", maxHeight: "600px", overflowY: "auto", overflowX: "auto", scrollbarWidth: "thin", scrollbarColor: "#2575fc #e6f0fa" }}>
          <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0" }}>
            <thead style={{ background: "linear-gradient(135deg, #2575fc, #6a11cb)", color: "white", position: "sticky", top: 0, zIndex: 2, boxShadow: "0 2px 10px rgba(0,0,0,0.2)" }}>
              <tr>
                {["Order ID", "Customer Name", "Contact No", "Product Details", "Approval Status", "Delivery Date", "Actions"].map((header) => (
                  <th key={header} style={{ padding: "18px 15px", fontSize: "0.95rem", fontWeight: "600", textTransform: "uppercase", letterSpacing: "0.5px", borderBottom: "2px solid rgba(255,255,255,0.2)", whiteSpace: "nowrap" }}>
                    {header}
                  </th>
                ))}
              </tr>
            ...
            <tbody>
              {filteredOrders.length > 0 ? (
                filteredOrders.map((order) => (
                  <tr key={order._id} style={{ backgroundColor: "#ffffff", transition: "all 0.3s ease", borderBottom: "1px solid #e6f0fa" }}>
                    <td style={{ padding: "15px" }}>{order.orderId || "-"}</td>
                    <td style={{ padding: "15px" }}>{order.customername || "-"}</td>
                    <td style={{ padding: "15px" }}>{order.contactNo || "-"}</td>
                    <td style={{ padding: "15px" }}>{order.products ? order.products.map((p) => `${p.productType} (${p.qty})`).join(", ") : "-"}</td>
                    <td style={{ padding: "15px" }}>
                      <Badge bg={order.sostatus === "Pending for Approval" ? "warning" : "info"} style={{ padding: "6px 12px", fontSize: "0.9rem" }}>
                        {order.sostatus || "-"}
                      </Badge>
                    </td>
                    <td style={{ padding: "15px" }}>{order.deliveryDate ? new Date(order.deliveryDate).toLocaleDateString("en-GB") : "-"}</td>
                    <td style={{ padding: "15px", textAlign: "center" }}>
                      <div style={{ display: "flex", gap: "10px" }}>
                        <Button variant="primary" onClick={() => handleViewClick(order)} style={{ width: "40px", height: "40px", borderRadius: "50%", padding: "0", display: "flex", alignItems: "center", justifyContent: "center" }}>
                          <FaEye />
                        </Button>
                ...
                <tr>
                  <td colSpan="7" style={{ padding: "20px", textAlign: "center", fontStyle: "italic", color: "#6b7280" }}>No orders found.</td>
                </tr>
              )}
            </tbody>
          </table>
      ...
      {isEditModalOpen && (
        <EditProductionApproval isOpen={isEditModalOpen} onClose={() => setIsEditModalOpen(false)} onEntryUpdated={handleEntryUpdated} entryToEdit={selectedOrder} />
      )}
    </>
  );
};
