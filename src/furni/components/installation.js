...
function Installation() {
  ...
  const fetchInstallationOrders = useCallback(async (page = currentPage, limit = pageLimit) => {
    ...
    try {
      ...
      const response = await furniApi.get(`/api/installation-orders?${params.toString()}`);
  ...
  }, [currentPage, pageLimit, searchQuery, startDate, endDate, statusFilter, salesPersonFilter]);

  useEffect(() => {
    let isMounted = true;
    fetchInstallationOrders(currentPage, pageLimit).then(() => { if (!isMounted) setInitialLoading(false); });
  ...
  const handleExportExcel = async () => {
    try {
      ...
      // Add all active filters to export query (only filters that exist in this component)
      if (searchQuery) params.append('search', searchQuery);
      if (startDate) params.append('startDate', startDate);
      if (endDate) params.append('endDate', endDate);
      if (statusFilter && statusFilter !== 'All') params.append('status', statusFilter);
          ...
          <div style={{ overflowX: "auto", maxHeight: "550px", border: "1px solid rgba(0, 0, 0, 0.1)", borderRadius: "8px", backgroundColor: "rgba(255, 255, 255, 0.8)", boxShadow: "0 4px 8px rgba(0, 0, 0, 0.1)", backdropFilter: "blur(10px)", position: "relative" }}>
            <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0" }}>
              ...
              <tbody>
                {tableLoading ? (
                  <tr>
                    <td colSpan="11" style={{ padding: "40px", textAlign: "center", background: "linear-gradient(135deg, #f8f9fa, #e9ecef)" }}>
                  ...
                  <tr>
                    <td colSpan="11" style={{ padding: "40px", textAlign: "center", color: "#6b7280", fontStyle: "italic", background: "linear-gradient(135deg, #f8f9fa, #e9ecef)" }}>
                      No installation orders available at this time.
                    </td>
                  </tr>
                ) : (
