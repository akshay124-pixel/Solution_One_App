...
function EditEntry({ isOpen, onClose, onEntryUpdated, entryToEdit }) {
  ...
  const handleDownload = async (filePath) => {
    ...
    try {
      ...
      const response = await fetch(fileUrl, { method: "GET", headers: { Accept: "application/pdf,image/png,image/jpeg,image/jpg,application/msword,application/vnd.openxmlformats-officedocument.wordprocessingml.document,application/vnd.ms-excel,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" } });
  ...
  const renderEditForm = () => (
    <Form onSubmit={handleSubmit(onEditSubmit)}>
      <FormSection>
        ...
        <Form.Group controlId="alterno"><Form.Label>\ud83d\udcde Alternate Contact Number</Form.Label><Form.Control {...register("alterno", { pattern: { value: /^\d{10}$/, message: "Alternate contact number must be exactly 10 digits" }, maxLength: { value: 10, message: "Alternate contact number must be exactly 10 digits" } })} onChange={(e) => { const value = e.target.value.replace(/\D/g, "").slice(0, 10); e.target.value = value; debouncedHandleInputChange("alterno", value); }} onKeyDown={(e) => { if (e.key === " " || (!/\d/.test(e.key) && !["Backspace", "Delete", "ArrowLeft", "ArrowRight", "Tab"].includes(e.key))) e.preventDefault(); }} maxLength={10} isInvalid={!!errors.alterno} placeholder="Enter 10 digit number" /><Form.Control.Feedback type="invalid">{errors.alterno?.message}</Form.Control.Feedback></Form.Group>
